...
class ProteinTask:
    ...
    def set_input_protein_task(
        self, protein_path=None, chains=None, regions=None, task=None
    ):
        if task is None:
    ...
    def set_wildtype_of_protein(
        self, of_features, of_protein=pd.DataFrame(), of_cycles=None
    ):
        ...
        if obs_positions is None:
            ...
            for chain in chains:
                start = min(
                    ...
                    (prot_wt["chain_id_original"] == chain)
    ...
    def get_index_for_observable_position(self, obs_pos, protein_mt, mutations=None):
        ...
        resi_obs = list(
            range(resi_obs_start, resi_obs_start + obs_pos["shift_from_resi"] + 1)
        )
        for idx in resi_obs:
...
def get_feature_tensor(task, feature_names=["pair", "single", "plddt", "lddt_logits"]):
    # Check feature names
    for f_name in feature_names:
        assert f_name in [
            ...
            "single",
            "tm_logits",
        ], f"Unknown feature name: {f_name}! Known feature names: 'msa', 'pair', 'lddt_logits', 'distogram_logits', 'aligned_confidence_probs',\
            'predicted_aligned_error', 'plddt', 'single', 'tm_logits'."
...
def get_feature_separate_tensors(
    task, feature_names=["pair", "single", "plddt", "lddt_logits"]
):
    # Check feature names
    for f_name in feature_names:
        assert f_name in [
